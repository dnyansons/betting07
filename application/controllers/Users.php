...
class Users extends CI_Controller {
    ...
    public function block_user() {
        ...
        $this->load->view('admin/common/header', $data);
        $this->load->view('admin/common/sidebar');
        $this->load->view('admin/user/block_list');
        $this->load->view('admin/common/footer');
    }
    ...
    public function ajax_list() {
        ...
        $totalData = $this->User_model->all_user_count_playing(0);

        $totalFiltered = $totalData;

        if (empty($this->input->post('search')['value'])) {
            $users = $this->User_model->all_user_playing($limit, $start, $col, $dir, 0);
            // echo $this->db->last_query();
        } else {
            $search = $this->input->post('search')['value'];

            $users = $this->User_model->user_search_playing($limit, $start, $search, $col, $dir, 0);
            // echo $this->db->last_query();
            $totalFiltered = $this->User_model->user_search_count_playing($search, 0);
        }
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $dat) {
                ...
                $nestedData['action'] = '&nbsp;&nbsp;&nbsp;<a class="text-muted font-16" href="' . base_url() . 'users/edit/' . $dat->user_id . '"><i class="ti-pencil-alt"></i></a>';
                $data[] = $nestedData;
            }
        }
    ...
    //Block User Ajax
    public function ajax_block_list() {
        $columns = array(
            0 => 'user_id',
            1 => 'username',
            2 => 'first_name',
        ...
        $totalData = $this->User_model->all_user_count_playing(1);

        $totalFiltered = $totalData;

        if (empty($this->input->post('search')['value'])) {
            $users = $this->User_model->all_user_playing($limit, $start, $col, $dir, 1);
            // echo $this->db->last_query();
        } else {
            $search = $this->input->post('search')['value'];

            $users = $this->User_model->user_search_playing($limit, $start, $search, $col, $dir, 1);
            // echo $this->db->last_query();
            $totalFiltered = $this->User_model->user_search_count_playing($search, 1);
        }
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $dat) {
                if (empty($dat->under)) {
                    ...
                    //get_name
                    $nm = $this->User_model->get_admin_agent($dat->under);
                    if (empty($nm[0]->username)) {
                        $unser = '--';
                    } else {
                        $unser = $nm[0]->username;
    ...
    public function add() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($check_mobile > 0) {
                ...
                $insertdata['role'] = $this->input->post('role');
                $insertdata['addr1'] = $this->input->post('addr1');
                $insertdata['addr2'] = $this->input->post('addr2');
                $insertdata['status'] = $this->input->post('status');
                $insertdata['commission'] = $this->input->post('commission'); //Agent Commsion
                $insertdata['created_by'] = $this->input->post('created_by'); //Under User
                ...
                $dat['user_id'] = 1;
                $dat['title'] = 'New' . $role . ' Joined';
                $dat['message'] = $this->input->post('username') . ' Joined as ' . $role;
    ...
    public function edit($id = 0) {
        ...
        if ($id != 0) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $data['users'] = $this->User_model->get_user_detail($id);
                $data['admin_agent'] = $this->User_model->get_admin_agent();
                $data['user_deatil'] = $this->Common_model->getAll('users', array('user_id' => $user_id))->result();
                $data['tot_wallet_bal'] = $this->Wallet_model->getTotalWalletBal();
                $data['role'] = $this->Common_model->getAll('role', array('status' => 'Active'))->result();
                ...
                $updata['role'] = $this->input->post('role');
                ...
                $updata['commission'] = $this->input->post('commission');
                $updata['created_by'] = $this->input->post('created_by'); //Under User
