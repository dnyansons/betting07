...
class Wallet extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view('agent/wallet/list', $data);
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $dat) {
                ...
                $nestedData['action'] = '<a href="#" user-id="' . $dat->user_id . '" data-toggle="modal" data-target="#wallet_model" class="tabledit-edit-button btn btn-info waves-effect waves-light btn-sm wallet_data" data-toggle="tooltip" title="Credit/Debit" >C / D</a>&nbsp;'
                        . '<a href="' . base_url() . 'agent/wallet/wallet_history/' . $dat->user_id . '" class="tabledit-edit-button btn btn-primary waves-effect waves-light btn-sm" data-toggle="tooltip" title="History" ><i class="fa fa-history" aria-hidden="true"></i></a>';
                $data[] = $nestedData;
                $i++;
            }
        }
    ...
    function wallet_history($user_id = 0) {
        ...
        if ($user_id !== '') {
            ...
            $this->load->view('agent/common/header', $data);
            $this->load->view('agent/common/sidebar');
            $this->load->view('agent/wallet/wallet_history', $data);
            $this->load->view('agent/common/footer');
        } else {
            show_error('Direct script access not allowed !');
        }
    ...
    public function ajax_wallet_historylist() {
        ...
        $totalData = $this->Wallet_model->all_wallet_hisory();
