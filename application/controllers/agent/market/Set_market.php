...
class Set_market extends CI_Controller {
    ...
    public function index() {
        ...
        $this->load->view('agent/market/set_market_list');
        ...
        if (!empty($markets)) {
            ...
            foreach ($markets as $mar) {
                ...
                if ($mar->m_status == 'Active') {
                    $status = '<a href="#" class="badge badge-success badge-pill text-white">' . $mar->m_status . '</a>';
                    $action = '<a href="' . base_url() . 'agent/market/set_market/view_market/' . $mar->m_id . '" class="btn btn-sm btn-warning btn-rounded">View Bets</a>';
                } elseif ($mar->m_status == 'Inactive') {
                    $status = '<a href="#" class="badge badge-danger  badge-pill text-white">' . $mar->m_status . '</a>';
                    $action = '<a href="' . base_url() . 'agent/market/set_market/view_market/' . $mar->m_id . '" class="btn btn-sm btn-warning btn-rounded">View Bets</a>';
                } else {
                    $status = '<a href="#" class="badge badge-pink badge-pill text-white">' . $mar->m_status . '</a>';
                    $action = '--';
                }
...
<select name="m_status" class="form-control">';
    ...
    function view_market($m_id = 0) {
        ...
        if ($m_id > 0) {
            ...
            if ($data['market']) {
                ...
                $this->load->view('agent/common/header', $data);
                $this->load->view('agent/common/sidebar');
                $this->load->view('agent/market/view_market', $data);
                $this->load->view('agent/common/footer');
            } else {
                redirect("market/set_market");
            }
    ...
    function user_market_bets($fancy_id = 0) {
        ...
        if ($ch > 0) {
            ...
            $this->load->view('agent/common/header', $data);
            $this->load->view('agent/common/sidebar');
            $this->load->view('agent/market/view_bets', $data);
            $this->load->view('agent/common/footer');
        } else {
            redirect("market/set_market");
        }
